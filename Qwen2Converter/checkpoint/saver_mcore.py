...
def save_checkpoint(queue, args):
    ...
    def pad_weight(orig_word_embed, true_vocab_size):
        if true_vocab_size is not None:
            ...
            margs.padded_vocab_size = 151936 #_vocab_size_with_padding(true_vocab_size, margs)
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for layer_id in range(len(setter.get_transformer_block(models[pp_rank][0][0]).layers)):
            ...
            if margs.moe_shared_expert_intermediate_size:
                if md.swiglu:
                    shared_mlp_l0_weight_W = chunk_weight(msg.pop("shared mlp l0 weight W"), "column", args.target_tensor_parallel_size)
                    shared_mlp_l0_weight_V = chunk_weight(msg.pop("shared mlp l0 weight V"), "column", args.target_tensor_parallel_size)
                    shared_mlp_l0_weight = torch.cat((shared_mlp_l0_weight_W, shared_mlp_l0_weight_V), dim=-2)
                else :
                    shared_mlp_l0_weight = chunk_weight(msg.pop("shared mlp l0 weight"), "column", args.target_tensor_parallel_size)
                shared_mlp_l1_weight = chunk_weight(msg.pop("shared mlp l1 weight"), "row", args.target_tensor_parallel_size)
